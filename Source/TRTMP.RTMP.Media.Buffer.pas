...
type
  TRtmpPinnedTrackPackets = array[0..255] of TRtmpPacket;

  TRtmpCircularBuffer = class
  private
    FItems: TObjectList;
    FLock: TCriticalSection;
    FLatestAudioConfigs: TRtmpPinnedTrackPackets;
    FLatestKeyframe: TRtmpPacket;
    FLatestMetadata: TRtmpPacket;
    FLatestVideoConfigs: TRtmpPinnedTrackPackets;
    ...
    function HasPinnedConfigLocked(const AConfigs: TRtmpPinnedTrackPackets): Boolean;
    procedure PinCodecConfig(const APacket: TRtmpPacket; AAudio: Boolean);
  ...
  public
    ...
    function HasAudioTrackConfig(ATrackID: Integer): Boolean;
    function HasVideoTrackConfig(ATrackID: Integer): Boolean;
...
begin
  ...
  try
    ...
    for I:=0 to 255 do
    begin
      FreeAndNil(FLatestAudioConfigs[I]);
      FreeAndNil(FLatestVideoConfigs[I]);
    end;
...
procedure SortPacketsBySequence(var APackets: TRtmpPacketArray); forward;
...
begin
  ...
  try
    ...
    begin
      SetLength(Result, 1);
      Result[0]:=FLatestMetadata;
    end;
    for I:=0 to 511 do
    begin
      if I <= 255 then
        Packet:=FLatestAudioConfigs[I]
      else
        Packet:=FLatestVideoConfigs[I - 256];
      if Packet = nil then
        Continue;
      Seen:=False;
      for J:=0 to High(Result) do
        if (Result[J].SequenceNo = Packet.SequenceNo) AND
          (Result[J].MessageType = Packet.MessageType) then
        begin
          Seen:=True;
          Break;
        end;
      ...
      begin
        ...
        Result[High(Result)]:=Packet;
      end;
    end;
    SortPacketsBySequence(Result);
  finally
    FLock.Release;
  end;
end;

procedure SortPacketsBySequence(var APackets: TRtmpPacketArray);
var
  I: Integer;
  J: Integer;
  Temp: TRtmpPacket;
begin
  for I:=1 to High(APackets) do
  begin
    J:=I;
    while (J > 0) AND
      (APackets[J - 1].SequenceNo > APackets[J].SequenceNo) do
    begin
      Temp:=APackets[J - 1];
      APackets[J - 1]:=APackets[J];
      APackets[J]:=Temp;
      Dec(J);
    end;
  end;
...
begin
  ...
  try
    ...
    begin
      SetLength(Result, 1);
      Result[0]:=FLatestMetadata.CloneShallow;
    end;
    for I:=0 to 511 do
    begin
      if I <= 255 then
      ...
      begin
        SetLength(Result, Length(Result) + 1);
        Result[High(Result)]:=Packet.CloneShallow;
      end;
    end;
    SortPacketsBySequence(Result);
  finally
...
begin
  ...
  for I:=0 to 255 do
    if FLatestAudioConfigs[I] <> nil then
    begin
      Unique:=True;
      for J:=0 to I - 1 do
        if (FLatestAudioConfigs[J] <> nil) AND
          (FLatestAudioConfigs[J].SequenceNo = FLatestAudioConfigs[I].SequenceNo) then
        begin
          Unique:=False;
          Break;
        end;
      if Unique then
        Inc(Result, UInt64(FLatestAudioConfigs[I].PayloadSize));
    end;
  for I:=0 to 255 do
    if FLatestVideoConfigs[I] <> nil then
    begin
      Unique:=True;
      for J:=0 to I - 1 do
        if (FLatestVideoConfigs[J] <> nil) AND
          (FLatestVideoConfigs[J].SequenceNo = FLatestVideoConfigs[I].SequenceNo) then
        begin
          Unique:=False;
          Break;
        end;
      if Unique then
        Inc(Result, UInt64(FLatestVideoConfigs[I].PayloadSize));
    end;
  if FLatestKeyframe <> nil then
...
function TRtmpCircularBuffer.HasPinnedConfigLocked(
  const AConfigs: TRtmpPinnedTrackPackets): Boolean;
var
  I: Integer;
begin
  Result:=False;
  for I:=0 to 255 do
    if AConfigs[I] <> nil then
      Exit(True);
end;

function TRtmpCircularBuffer.HasAudioTrackConfig(ATrackID: Integer): Boolean;
begin
  Result:=False;
  if (ATrackID < 0) OR (ATrackID > 255) then
    Exit;
  FLock.Acquire;
  try
    Result:=FLatestAudioConfigs[ATrackID] <> nil;
  finally
    FLock.Release;
  end;
end;

function TRtmpCircularBuffer.HasVideoTrackConfig(ATrackID: Integer): Boolean;
begin
  Result:=False;
  if (ATrackID < 0) OR (ATrackID > 255) then
    Exit;
  FLock.Acquire;
  try
    Result:=FLatestVideoConfigs[ATrackID] <> nil;
  finally
    FLock.Release;
  end;
end;

procedure TRtmpCircularBuffer.PinCodecConfig(const APacket: TRtmpPacket;
  AAudio: Boolean);
var
  I: Integer;
...
begin
  ...
  if AAudio then
    Parsed:=RtmpInspectFlvTag(mtAudio, APacket.Payload.Bytes, Info)
  else
    Parsed:=RtmpInspectFlvTag(mtVideo, APacket.Payload.Bytes, Info);

  if Parsed AND Info.IsMultitrack AND (Info.TrackCount > 0) then
  begin
    for I:=0 to Info.TrackCount - 1 do
      if AAudio then
        ReplacePinnedPacket(FLatestAudioConfigs[Info.Tracks[I].TrackID], APacket)
      else
        ReplacePinnedPacket(FLatestVideoConfigs[Info.Tracks[I].TrackID], APacket);
  end
  else if AAudio then
    ReplacePinnedPacket(FLatestAudioConfigs[0], APacket)
  else
    ReplacePinnedPacket(FLatestVideoConfigs[0], APacket);
end;
...
begin
  ...
  Result.HasAudioConfig:=HasPinnedConfigLocked(FLatestAudioConfigs);
  Result.HasVideoConfig:=HasPinnedConfigLocked(FLatestVideoConfigs);
  Result.AudioConfigTrackCount:=0;
  Result.VideoConfigTrackCount:=0;
  for I:=0 to 255 do
  begin
    if FLatestAudioConfigs[I] <> nil then
      Inc(Result.AudioConfigTrackCount);
    if FLatestVideoConfigs[I] <> nil then
      Inc(Result.VideoConfigTrackCount);
  end;
