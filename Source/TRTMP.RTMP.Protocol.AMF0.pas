...
type
  ...
  TRtmpAmf0StrictArray = class(TRtmpAmf0Value)
  private
    FItems: TObjectList;
    function GetCount: Integer;
  ...
  public
    constructor Create;
    destructor Destroy; override;
    function Add(AValue: TRtmpAmf0Value): Integer;
    function Clone: TRtmpAmf0Value; override;
  ...
  private
    ...
    class function ReadStrictArray(AReader: TRtmpByteReader): TRtmpAmf0StrictArray; static;
...
constructor TRtmpAmf0StrictArray.Create;
begin
  inherited Create(avStrictArray);
  FItems:=TObjectList.Create(True);
end;

destructor TRtmpAmf0StrictArray.Destroy;
begin
  FItems.Free;
  inherited Destroy;
end;

function TRtmpAmf0StrictArray.Add(AValue: TRtmpAmf0Value): Integer;
begin
  Result:=FItems.Add(AValue);
end;

function TRtmpAmf0StrictArray.Clone: TRtmpAmf0Value;
var
  I: Integer;
  NewArray: TRtmpAmf0StrictArray;
begin
  NewArray:=TRtmpAmf0StrictArray.Create;
  for I:=0 to FItems.Count - 1 do
    if FItems[I] = nil then
      NewArray.Add(nil)
    else
      NewArray.Add(TRtmpAmf0Value(FItems[I]).Clone);
  Result:=NewArray;
end;

function TRtmpAmf0StrictArray.GetCount: Integer;
begin
  Result:=FItems.Count;
end;

function TRtmpAmf0StrictArray.GetItem(Index: Integer): TRtmpAmf0Value;
begin
  Result:=TRtmpAmf0Value(FItems[Index]);
end;
...
begin
  ...
  case Marker of
    ...
    Byte(amfStrictArray):
      Result:=ReadStrictArray(AReader);
    Byte(amfLongString):
...
begin
  ...
  case AValue.Kind of
    ...
    avStrictArray:
      begin
        AWriter.WriteUInt8(MarkerByte(amfStrictArray));
        AWriter.WriteUInt32BE(TRtmpAmf0StrictArray(AValue).Count);
        for I:=0 to TRtmpAmf0StrictArray(AValue).Count - 1 do
          EncodeValue(AWriter, TRtmpAmf0StrictArray(AValue)[I]);
      end;
  else
...
class function TRtmpAmf0.ReadStrictArray(
  AReader: TRtmpByteReader): TRtmpAmf0StrictArray;
var
  Count: UInt32;
  I: Integer;
begin
  Count:=AReader.ReadUInt32BE;
  if Count > UInt32(AReader.Remaining) then
    raise ERtmpAmf0Error.CreateFmt(
      'AMF0 strict array count %d exceeds remaining payload size %d',
      [Count, AReader.Remaining]);

  Result:=TRtmpAmf0StrictArray.Create;
  try
    for I:=0 to Integer(Count) - 1 do
      Result.Add(DecodeValue(AReader));
  except
    Result.Free;
    raise;
  end;
