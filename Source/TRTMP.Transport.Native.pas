...
type
  IRtmpNativeConnectionAccess = interface(IInterface)
    ['{DA76349B-D973-42B9-956F-B8064ACDA3D6}']
    function GetNativeHandle: NativeUInt;

    property NativeHandle: NativeUInt read GetNativeHandle;
  end;
...
begin
  if ABlocking then
    ...
    NonBlocking:=1;
  Result:=ioctlsocket(ASocket, RTMP_FIONBIO, NonBlocking) = 0;
end;
{$ELSE}
var
  Flags: LongInt;
...
function TRtmpNativeConnection.GetNativeHandle: NativeUInt;
begin
  Result:=NativeUInt(FSocket);
end;
